    ...
    render() {
        ...
        let form = (
            <>
                <Form onSubmit={event => this.submitHandler(event)}>
                    {formContent}
                    <FormGroup>
                        <Button color="danger">Créer<FontAwesomeIcon icon="folder-plus" size="lg" className="ml-1" /></Button>
                    </FormGroup>
                </Form>
            </>
        );
        ...
        return (
            <Col xs={12} className="p-0">
                {redirect}
                <Breadcrumb main="Ajouter un rôle" icon="user-tag" />
                <PresentationalContainer user bg="light">
                    <SpecialTitle user icon="user-tag">Ajouter un rôle</SpecialTitle>
                    <Subtitle>Ici, vous pouvez créer un nouveau rôle et l'ajouter à l'application.</Subtitle>
                    {errors}
                    {form}
...
const mapDispatchToProps = dispatch => ({
    ...
    onStoreRole: data => dispatch(actions.adminStoreRole(data)),
});
