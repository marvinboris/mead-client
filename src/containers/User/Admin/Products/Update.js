...
import { Col, Form, FormGroup, Button, Spinner, Row } from 'reactstrap';
...
class Update extends Component {
    ...
    render() {
        ...
        let redirect = null;
        if (formSubmitted && !(formLoading || authLoading || adminLoading) && !(formError || authError || adminError)) redirect = <Redirect to="/admin/products" />
            ...
            form = (
                <>
                    <Form onSubmit={event => this.submitHandler(event)}>
                        <Row>
                            <Col md={4}>
                                <img src={"http://localhost:8080/" + product.imageUrl} alt={product.title} className="img-fluid img-responsive rounded img-thumbnail" />
                            </Col>
                            <Col md={8}>
                                {formContent}
                                <FormGroup>
        ...
        return (
            <Col xs={12} className="p-0">
                {redirect}
                <Breadcrumb main="Modifier un produit" icon="edit" />
