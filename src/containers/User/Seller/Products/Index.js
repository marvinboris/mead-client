    ...
    render() {
        ...
        if (productLoading) content = (
            ...
            if (products) {
                const productsData = products.map(product => updateObject(product, {
                    ...
                    title: <NavLink to={"/community/products/" + product._id}>{product.title}</NavLink>,
                    ...
                    <Row>
                        <Table array={productsData} select selectHandler={this.allSelectedHandler} className="bg-white border rounded shadow" title="Liste des produits" bordered xl={12} fields={[
                            ...
                            <div className="text-right py-2">
                                <ButtonGroup>
                                    <NavLink className="btn btn-info btn-sm" to="/community/products/create"><FontAwesomeIcon icon="plus" fixedWidth className="mr-1" />Ajouter</NavLink>
        ...
        return (
            <Col xs={12} className="p-0">
                <Breadcrumb main="Produits" icon={["fab", "product-hunt"]} items={[{ to: '/community', content: 'Ma communauté' }]} />
                <PresentationalContainer user bg="light">
                    <SpecialTitle user icon={["fab", "product-hunt"]}>Produits</SpecialTitle>
                    <Subtitle>Ici, vous pouvez trouver la liste de tous vos produits.</Subtitle>
                    {errors}
                    {content}
